...
class BlockRegistry
{
  ...
  {
    \Diagnostic\Features\Scanner\Core\WPLog::debug('Vérification du bloc: ' . $blockName, '[BlockRegistry]');
    if (empty($blockName)) {
      return false;
    }
  ...
  {
    \Diagnostic\Features\Scanner\Core\WPLog::debug('Récupération des post types valides', '[BlockRegistry]');
